...
NodeViewer.prototype = {
  ...
  createTopGoal: function(idx, str) {
    var top = new Goal();
    top.init(null, idx, 'goal' + this.goalID,
             this.w / 2 - 110, 50, str);
    this.goalID++;
    this.root = top;
    return top;
  ...
  createGoal: function(idx, parent, str) {
    var node = new Goal();
    node.init(parent, idx, 'goal' + this.goalID, parent.x, parent.y, str);
    node.hide();
    this.goalID++;
  ...
  createStrategy: function(idx, parent, str) {
    ...
    console.log('parent: x: ' + parent.x + ', y: ' + parent.y);
    node.init(parent, idx, 'str' + this.strID, parent.x, parent.y, str);
    node.hide();
    this.strID++;
    parent.addChild(node);
    parent.connectTo(node);
  ...
  createEvidence: function(idx, parent, str) {
    ...
    node.init(parent, idx, 'evi' + this.eviID, parent.x, parent.y, str);
    node.hide();
    this.eviID++;
    parent.addChild(node);
    parent.connectTo(node);
  ...
  createContext: function(idx, str) {
    var node = new Goal();
    node.init(parent, idx, 'ugoal' + this.goalID, parent.x, parent.y, str);
    node.hide();
    this.goalID++;
    parent.addChild(node);
    parent.connectTo(node);
...
function sendDummyGoal(str) {
  setTimeout(function() {
    ...
    $('#dcase').addClass('animated fadeInDown');
    $('#dcase_node').addClass('animated fadeInDown');
  }, 1500);
}

...
Node.prototype = {
  init: function(svg, parent, idx, id, x, y, str) {
    this.svg = svg;
    this.idx = idx;
    this.x = x;
    this.y = y;
  ...
  connectTo: function(node) {
    var l = this.svg.line(line, this.bottom['x'], this.bottom['y'],
...
$.extend(Goal.prototype, Node.prototype, {
  init: function(svg, parent, idx, id, x, y, str) {
    ...
    this.bottom['x'] = x + w / 2;
    this.bottom['y'] = y + h;
    this.node = createLastChild($('#dcase_node'));
    this.node.addClass('goal');
    //this.padY = $(this.svg).offset().top;
    this.node.css({
      'left': this.x + 'px',
      'top': raw_y(this.y) + 'px'
    });
    ...
    this.id = id;
    this.rowtext = str;
    this.text = createLastChild($(this.node), 'p', str);
    this.text.attr('id', 'text');
    this.idtxt = createLastChild($(this.node), 'p', 'ID: ' + id);
    this.idtxt.attr('id', 'idtext');
    this.enableEvent();
    this.setText();
    this.dimd = createLastChild($(this.node));
    this.dimd.addClass('ugoaldiamond');
...
$.extend(Context.prototype, Node.prototype, {
  init: function(svg, parent, idx, id, x, y, str) {
    ...
    $(this.g).attr('id', id);
    this.id = id;
    this.text = this.svg.createText(); // [TODO]
